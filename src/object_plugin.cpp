...
void ObjectPlugin<PointT>::setAndLoadObjectDatabaseLocation(
  ...
  // (&ObjectPlugin<PointT>::objectDiscoveryLoop, this);
...
void ObjectPlugin<PointT>::loadDatabase() {
  ...
  // pcl::UniformSampling<PointT>* uniform_sampling_ = new
  // pcl::UniformSampling<PointT>; uniform_sampling_->setRadiusSearch(0.01f);

  boost::shared_ptr<pcl::Keypoint<PointT, pcl::PointXYZI> > keypoint_detector;
  keypoint_detector.reset(sift3D);

  /*
   typename pcl::Feature<PointT, pcl::VFHSignature308>::Ptr feature_extractor
   (new pcl::CVFHEstimation<PointT, pcl::Normal, pcl::VFHSignature308>);
   feature_extractor->setSearchMethod (typename pcl::search::Search<PointT>::Ptr
   (new pcl::search::KdTree<PointT>));
   //feature_extractor->setRadiusSearch (0.05);
   typename pcl::PointCloud<pcl::VFHSignature308>::Ptr features(new
  ...
  // PFHRGB
  /*
   typename pcl::Feature<PointT, pcl::PFHRGBSignature250>::Ptr feature_extractor
   (new pcl::PFHRGBEstimation<PointT, pcl::Normal, pcl::PFHRGBSignature250>);
   feature_extractor->setSearchMethod (typename pcl::search::Search<PointT>::Ptr
   (new pcl::search::KdTree<PointT>)); feature_extractor->setRadiusSearch
   (0.05); typename pcl::PointCloud<pcl::PFHRGBSignature250>::Ptr features(new
   pcl::PointCloud<pcl::PFHRGBSignature250>);
   */

  // FPFH
  /*
   pcl::Feature<PointT, pcl::FPFHSignature33>::Ptr feature_extractor (new
   pcl::FPFHEstimationOMP<PointT, pcl::Normal, pcl::FPFHSignature33>);
   feature_extractor->setSearchMethod (pcl::search::Search<PointT>::Ptr (new
   pcl::search::KdTree<PointT>)); feature_extractor->setRadiusSearch (0.05);
  ...
  // object_recognition_.reset(new
  // ObjectRecognition<pcl::FPFHSignature33>(keypoint_detector,
  // feature_extractor));
  object_recognition_.reset(new ObjectRecognition<pcl::SHOT1344>(
      keypoint_detector, feature_extractor));

  if (debug_) std::cout << "[ObjectPlugin] Descriptor Loaded " << std::endl;
  /* load object descriptors */

  if (verbose_)
    std::cout << "[ObjectPlugin] Object location: " << object_database_location_
              << std::endl;
...
void ObjectPlugin<PointT>::reconstructSurface(CloudPtr merged, int id) {
  ...
  // apply grid filtering to reduce amount of points as well as to make them
  // uniform distributed
  ...
  pcl::PointCloud<pcl::Normal>::Ptr vertices(new pcl::PointCloud<pcl::Normal>);
  pcl::copyPointCloud(*merged, *vertices);

  pcl::NormalEstimation<pcl::PointXYZRGBA, pcl::Normal> normal_estimation;
  normal_estimation.setSearchMethod(pcl::search::Search<pcl::PointXYZRGBA>::Ptr(
...
void ObjectPlugin<PointT>::computeTSDF(Object<PointT> object,
  ...
  /*
   const gtsam::Rot3 rot;
   const gtsam::Point3 centroid_pt(obj_centroid[0], obj_centroid[1],
   obj_centroid[2]); gtsam::Pose3 centroid_tform(rot, centroid_pt); gtsam::Pose3
   inv_tform = centroid_tform.inverse(); Eigen::Matrix4f map_tform =
   inv_tform.matrix().cast<float>();
      ...
      for (it_cluster = cluster.begin(); it_cluster != cluster.end();
        ...
        if (cloud_pose) {
          ...
          pcl::transformPointCloud(
              *cloud, *map_cloud,
              map_transform);  // transform cloud from base_frame to world_frame
          transformed_cloud_opt_ = transformed_cloud_opt_ + *map_cloud;
        }
      }

      object.optimal_cloud_ =
          transformed_cloud_opt_.makeShared();  // fill in the optimal cloud
      object.object_mutex_.unlock();            // unlock the object
    }

    boost::this_thread::sleep(
        boost::posix_time::milliseconds(100));  // sleep for 1000ms
  }
}
...
void ObjectPlugin<PointT>::recognizeObject(Object<PointT>& object) {
  ...
  if (debug_)
    std::cout << "[ObjectPlugin] Object " << id << " seen in " << cluster.size()
              << " frames" << std::endl;
  ...
  if (verbose_) std::cout << "[ObjectPlugin] Saving new model" << std::endl;
  ...
  if (verbose_) std::cout << "[ObjectPlugin] Model saved" << std::endl;
  ...
  if (debug_)
    std::cout << "[ObjectPlugin] Object " << id << " centroid is "
              << obj_centroid[0] << " " << obj_centroid[1] << " "
              << obj_centroid[2] << std::endl;
  ...
  if (!mapper_->getSolution().exists(obj_symbol) && !object.landmark) {
    ...
    gtsam::GenericValue<gtsam::Point3> object_centroid_pt_val(
        object_centroid_pt);
  ...
  for (it = cluster.begin(); it != cluster.end(); it++) {
    ...
    if (cloud_pose) {
      ...
      pcl::transformPointCloud(transformed_cloud_opt_, *map_cloud,
                               map_transform);
      pcl::io::savePCDFileASCII("/home/siddharth/kinect/map_cloud.pcd",
                                *map_cloud);
      ...
      std::cout << "[ObjectPlugin] untformed_centroid " << measurement_centroid
                << " tformed_centroid " << tform_measurement_centroid
                << " object_centroid " << object_cloud_centroid << std::endl;
      ...
      /*
            ...
            //  icp.setMaxCorrespondenceDistance (1.5);//1.5
              ...
              //pcl::io::savePCDFileASCII ("transformed_cloud.pcd",
         transformed_cloud_opt_); if (debug_) printf ("ICP completed...\n");
              tform = icp.getFinalTransformation ();
              score = icp.getFitnessScore ();
              std::cout << "[ObjectPlugin] has converged: " <<
         icp.hasConverged() << std::endl; std::cout << "[ObjectPlugin] Fitness
         Score " << score << std::endl; std::cout << "[ObjectPlugin] Transform "
         << tform << std::endl; Eigen::Matrix4d tform4d = tform.cast<double>();
              gtsam::Pose3 icp_tform(tform4d);

              // TODO: find the transformation from camera's view point and use
         the corresponding translation as measurmenent offset gtsam::Pose3
         composed_tform =  icp_tform * new_pose.inverse(); icp_tform.print("ICP
         Transform"); new_pose.print("Camera Pose");
      ...
      */
      ...
      //   measurement_centroid[1], measurement_centroid[2]); //centroid in the
      //   robot base frame

      const gtsam::Point3 measurement_pt(
          tform_measurement_centroid[0], tform_measurement_centroid[1],
      ...
      // object centroid Point3 is tranformed in the robot pose Pose3 and the
      // error is computed w.r.t measurement pt
  ...
  if (solution.exists(obj_symbol)) {
    try {
      ...
      if (debug_) std::cout << "[ObjectPlugin] Marginal computed" << std::endl;
      covariance_mat += mat;
    } catch (std::out_of_range) {
      if (debug_) {
        std::cout << "[ObjectPlugin] Out of range: " << std::endl;
        std::cout << "[ObjectPlugin] Do solution exists: "
                  << solution.exists(obj_symbol) << std::endl;
      }
    ...
    covariance_mat(2, 2) = 0.001;
    if (debug_)
      std::cout << "[ObjectPlugin] Covariance is " << covariance_mat
                << std::endl;
    //	return;
  }
  ...
  if (obj.first == -2) {
    ...
    if (debug_)
      std::cout << "[ObjectPlugin] Object " << id << " matched to " << obj.first
                << std::endl;
    ...
    if (obj.first < max_object_size) {
      ...
      gtsam::Point3 matched_centroid_pt(
      ...
      if (!mapper_->getSolution().exists(match_symbol)) {
        gtsam::GenericValue<gtsam::Point3> matched_centroid_pt_val(
            matched_centroid_pt);
  ...
  object_recognition_->saveMapping(object_database_location_ + "/mapping.txt");
  object_recognition_->saveObjectStats(object_database_location_ +
                                       "/stats.txt");
...
float ObjectPlugin<PointT>::computeViewIntersection(
  ...
  gtsam::Point3 obj_center(
      obj_centroid[0], obj_centroid[1],
      obj_centroid[2]);  // center of the object in world frame
  gtsam::Point3 obj_vector =
      obj_center - view_center;     // camera center to object center direction
  ...
  if (debug_)
    std::cout << "[ObjectPlugin] [computeViewIntersection] object angle: "
              << angle << std::endl;

  /* distance between object and camera view */
  double distance = norm;

  if (norm > 4 || angle > 60)  // object is either more than 4 meters away or
...
void ObjectPlugin<PointT>::clusterCloudCallback(
  ...
  if (get_sensor_to_base_) {
    if (debug_)
      printf(
          "[ObjectPlugin] Object Plugin: Applying sensor to base transform.\n");
  ...
  // observations_.insert (std::pair<gtsam::Symbol, CloudPtrVector
  // >(pose_symbol, clusters_base));
  ...
  for (int i = 0; i < clusters_base.size(); i++) {
    ...
    if (filter_points_near_planes_) {
      ...
      if (found_nearby_plane) {
        // gtsam::Plane<PointT> plane = solution.at<gtsam::Plane<PointT> >
        // (closest_sym);
        ...
        for (int j = 0; j < clusters_base[i]->points.size(); j++) {
          ...
          for (int k = 0; k < nearby_symbols.size(); k++) {
            ...
            if (ptp_dist <= ptp_pt_cull_thresh_) pt_ok = false;
    ...
    if (clusters_base[i]->points.size() >= min_points_) {
      ...
      double dist = sqrt(clust_centroid[0] * clust_centroid[0] +
      ...
      // Clusters that are too far away suffer from too much depth
      // discretization to be reliable
      bool dist_ok = dist < max_cluster_dist_;
      // Clusters that are too low in the robot frame can be excluded
      std::cout << "[ObjectPlugin] min_cluster_height " << min_cluster_height_
                << " centroid " << clust_centroid[2] << "\n";
      bool height_ok = clust_centroid[2] > min_cluster_height_;
      // Clusters with large bounding boxes are probably not objects of interest
      // -- these tend to architectural features
      bool bbox_volume_ok = bbox_volume < max_bbox_volume_;
      // Clusters that have some dimension that is quite large tend to be
      // uninteresting
      bool bbox_dims_ok = (size[0] < max_bbox_dim_) &&
                          (size[1] < max_bbox_dim_) &&
                          (size[2] < max_bbox_dim_);
      // Clusters with too low curvature are probably either planar objects that
      // were improperly segmented, or Also, if it is mostly planar, shape
      // descriptors will not be very reliable
      bool curvature_ok = curvature > min_curvature_;

      if (debug_) printf("[ObjectPlugin] centroid dist: %lf\n", dist);
      if (dist_ok && height_ok && bbox_volume_ok && bbox_dims_ok &&
        ...
        if (verbose_)
          std::cout << "[ObjectPlugin] Pushing cluster"
  ...
  observations_.insert(std::pair<gtsam::Symbol, CloudPtrVector>(
      pose_symbol, filtered_observations));
  ...
  if (cloud_pose && filtered_observations.size() != 0) {
    ...
    if (verbose_)
      std::cout << "[ObjectPlugin] Segmentation took: "
                << double(seg_end - seg_start) << std::endl;
  ...
  gtsam::Pose3 view_pose =
      (*cloud_pose) * gtsam::Pose3(sensor_to_base.matrix());
  ...
  gtsam::Point3 view_direction(
      view_direction_vec.x() / view_direction_norm,
      view_direction_vec.y() / view_direction_norm,
  ...
  if (verbose_)
    std::cout << "[ObjectPlugin] Visualizations took: "
              << double(vis_end - vis_start) << std::endl;
}
...
ObjectPlugin<PointT>::getObservations(gtsam::Symbol sym) {
  ...
  if (observations_.size() == 0) {
    if (verbose_) printf("[ObjectPlugin] returning empty due to size!\n");
    return (empty_);
  }

  if (verbose_) printf("[ObjectPlugin] Checking count!\n");
  if (observations_.count(sym) > 0) {
    ...
    if (verbose_)
      printf(
          "[ObjectPlugin] ObjectPlugin::getObservations: returning empty!\n");
    return (empty_);
  }
}
