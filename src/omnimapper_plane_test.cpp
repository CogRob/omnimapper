...
//   // Make a plane
//   gtsam::Plane<Point> plane(0.0, 0.0, -1.0, 1.0, boundary, empty_inliers,
//   centroid, empty_header);

//   pcl::PointCloud<PointT> initial_hull_ = plane.hull ();
// }

void testPlanarTransform() {
  // Make a Plane
  ...
  Eigen::Vector3d translation_part =
      new_norm3 * -1 * (new_norm[3] - plane_params[3]);
...
void testPlanarTransform2() {
  ...
  // viewer_.addPointCloud (boost::make_shared<pcl::PointCloud<PointT>
  // >(boundary1), "boundary1");
  ...
  pcl::transformPointCloud(boundary1, true_transformed_boundary1,
                           true_transform);
  pcl::computeMeanAndCovarianceMatrix(true_transformed_boundary1, cov1,
                                      centroid1);
  ...
  true_transformed_plane_params[3] =
      -1 * true_transformed_plane_params.dot(centroid1);
  ...
  // viewer_.addPointCloud (boost::make_shared<pcl::PointCloud<PointT>
  // >(true_transformed_boundary1), "true_boundary1");

  // Now compute the transform again, using the usual way
  Eigen::Vector3d orig_norm(plane_params[0], plane_params[1], plane_params[2]);
  Eigen::Vector3d new_norm(true_transformed_plane_params[0],
                           true_transformed_plane_params[1],
                           true_transformed_plane_params[2]);
  ...
  our_transformed_plane_params[3] =
      -1 * our_transformed_plane_params.dot(centroid);
...
int main(int argc, char** argv) {
  ...
  // (&no_motion_plugin); omb.addPosePlugin (no_motion_ptr); sleep (1);
  /*
    ...
    Eigen::aligned_allocator<pcl::PlanarRegion<PointT> > > t1_regions;
    std::vector<pcl::PlanarRegion<PointT>,
    Eigen::aligned_allocator<pcl::PlanarRegion<PointT> > > t2_regions;
    ...
    pcl::PlanarRegion<PointT> p1_region (p1_centroid, p1_cov, p1_count,
    p1_border.points, p1_coeffs); t1_regions.push_back (p1_region);
    ...
    pcl::PlanarRegion<PointT> p2_region (p2_centroid, p2_cov, p2_count,
    p2_border.points, p2_coeffs); t2_regions.push_back (p2_region);
    ...
    (1,3), cloud_tform (2,3))); gtsam::Pose3 relative_pose (gtsam::Rot3
    (cloud_tform.block (0, 0, 3, 3).cast<double>()), trans); double trans_noise
    = 100.0; double rot_noise = 100.0; gtsam::SharedDiagonal noise =
    gtsam::noiseModel::Diagonal::Sigmas (gtsam::Vector_ (6, rot_noise,
    rot_noise, rot_noise, trans_noise, trans_noise, trans_noise));
    gtsam::NonlinearFactor::shared_ptr between1 (new
    gtsam::BetweenFactor<gtsam::Pose3> (sym1, sym2, relative_pose, noise));
