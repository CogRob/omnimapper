      ...
      if (verify_ptp_dist)
      {
        ...
        for (int i = 0; i < hull_.points.size (); i++)
        {
          double ptp_dist = fabs (new_norm[0] * hull_.points[i].x +
                                  ...
                                  new_norm[2] * hull_.points[i].z +
                                  (d_+d (3)));
          double ptp_dist_new = fabs (new_norm[0] * map_hull_on_map.points[i].x +
                                      new_norm[1] * map_hull_on_map.points[i].y +
                                      new_norm[2] * map_hull_on_map.points[i].z +
                                      (d_+d (3)));
          orig_dist += ptp_dist;
...
*/
  ...
  //   if (!((a_ == prev_a_) && (b_ == prev_b_) && (c_ == prev_c_) && (d_ == prev_d_)))
  //   {
  //     // We need to generate a rotation from the previous coefficients to this one.
  //     Eigen::Vector3f new_normal (a_, b_, c_);
  //     Eigen::Vector3f prev_normal (prev_a_, prev_b_, prev_c_);
  //     double angle = acos (prev_normal.dot (new_normal));
  //     Eigen::Vector3f axis = prev_normal.cross (new_normal);
  ...
  void Plane<PointT>::Extend2 (const Pose3& pose, const gtsam::Plane<PointT>& plane)
  {
    ...
    // TODO: should we do this in the local frame to avoid lever-arm effect (see m-space paper)?
    Eigen::Affine3f map_to_pose = pose3ToTransform (pose);//(pose.matrix ().cast<float>());
    Eigen::Affine3f pose_to_map = map_to_pose.inverse ();
    //pcl::PointCloud<PointT> local_lm_hull = pcl::transformPointCloud (hull_, local_lm_hull, pose);
    //gtsam::Vector local_lm_coeffs = GetXo (pose);
    ...
    Eigen::Vector3d meas_norm (meas_coeffs_map[0], meas_coeffs_map[1], meas_coeffs_map[2]);
    ...
    if (debug_extend2)
    {
      ...
      for (int i = 0; i < meas_hull_aligned_map.points.size (); i++)
      {
        double ptp_dist1 = fabs (a_ * meas_hull_aligned_map.points[i].x +
                                 ...
                                 c_ * meas_hull_aligned_map.points[i].z +
                                 d_);
        double ptp_dist2 = fabs (a_ * meas_hull_aligned_map1.points[i].x +
                                 b_ * meas_hull_aligned_map1.points[i].y +
                                 c_ * meas_hull_aligned_map1.points[i].z +
                                 d_);
        hull1_dist += ptp_dist1;
        hull2_dist += ptp_dist2;
        //if (ptp_dist >= 0.001)
        //{
          //printf ("Error in Planeextend!  PTP dist not zero after aligning the meas: %lf\n", ptp_dist);
          //printf ("Point: %lf %lf %lf\n", meas_hull_aligned_map.points[i].x, meas_hull_aligned_map.points[i].y, meas_hull_aligned_map.points[i].z);
        //}
      }
      for (int i = 0; i < hull_.points.size (); i++)
      {
        double ptp_dist3 = fabs (a_ * hull_.points[i].x +
                                 b_ * hull_.points[i].y +
                                 c_ * hull_.points[i].z +
                                 d_);
    ...
    Eigen::Vector3d combined_centroid3d (combined_centroid[0], combined_centroid[1], combined_centroid[2]);
    ...
    pcl::transformPointCloud (origin_lm_hull, origin_xy_lm_hull, lm_to_z_transform);
    pcl::transformPointCloud (origin_meas_hull, origin_xy_meas_hull, lm_to_z_transform);
    ...
    if (debug_extend2)
    {
      ...
      for (int i = 0; i < hull_.points.size (); i++)
      {
        double ptp_dist = fabs (a_ * hull_.points[i].x +
                                b_ * hull_.points[i].y +
                                c_ * hull_.points[i].z +
                                d_);
          if (ptp_dist >= 0.001)
