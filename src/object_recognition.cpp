...
ObjectRecognition<FeatureType>::ObjectRecognition(
		boost::shared_ptr<pcl::Keypoint<pcl::PointXYZRGBA, pcl::PointXYZI> > keypoint_detector,
		typename pcl::Feature<pcl::PointXYZRGBA, FeatureType>::Ptr feature_extractor) :
		source_keypoints_(new pcl::PointCloud<pcl::PointXYZI>()), target_keypoints_(
				new pcl::PointCloud<pcl::PointXYZI>()), keypoint_detector_(
				keypoint_detector), feature_extractor_(feature_extractor), source_features_(
				new pcl::PointCloud<FeatureType>), target_features_(
				new pcl::PointCloud<FeatureType>), correspondences_(
				new pcl::Correspondences), show_source2target_(false), show_target2source_(
				false), show_correspondences(false), verbose_(true), debug_(
				true), pose_map(), object_centroid_map() {
...
template<typename FeatureType>
int ObjectRecognition<FeatureType>::matchToFile(typename pcl::PointCloud<pcl::PointXYZRGBA>::ConstPtr source, typename pcl::PointCloud<pcl::PointXYZRGBA>::ConstPtr target)
{
	int max_num_corr = 0;
	typename pcl::PointCloud<FeatureType> target_feature;
...
target_=target;
		...
		findCorrespondences(source_features_, target_features_,
				source2target_);
		findCorrespondences(target_features_, source_features_,
				target2source_);
		if (verbose_)
		...
		filterCorrespondences();
		if (verbose_)
			std::cout << "[After filtering]#Corr: "
					<< correspondences_->size() << std::endl;
...
std::pair<int, int> ObjectRecognition<FeatureType>::matchToFeatureFile(
		typename pcl::PointCloud<pcl::PointXYZRGBA>::ConstPtr source, int label, gtsam::Matrix C) {
	...
	if (source_keypoints_->size() == 0)
		return std::make_pair(max_num_corr, -2); // -2 = cannot find keypoints in the source cloud

	extractDescriptors(source_, source_keypoints_, source_features_);

	typename std::map<int, std::vector<pcl::PointCloud<FeatureType> > >::iterator feature_map_it;
	...
	for(int i=0 ; i <indices[0].size(); i++){
        ...
        gtsam::Point3 matching_pt(centroid_data[3*index], centroid_data[3*index+1], centroid_data[3*index+2]);
        gtsam::Point3 object_pt(feature_centroid[0], feature_centroid[1], feature_centroid[2]);
        ...
        gtsam::Point3 final_pt(Dcov.x()*D.x(), Dcov.y()*D.y(), Dcov.z()*D.z());
		...
		std::cout << "Distance to " << obj_id << " is " << dists[0][i] << std::endl;
	...
	for (feature_map_it = feature_map.begin();
		...
		for (int j = 0; j < obj_desc.size(); j++) {
			...
			if (verbose_)
				std::cout
						<< "finding correspondence between source " << label << " and feature file "
						<< obj_id  << " out of " << obj_desc.size() <<  " and " << indices[0].size() << std::endl;

			//	std::cout << "source feature size: " << source_features_->size() << " target_feature: " << target_feature.size() << std::endl;
			findCorrespondences(source_features_, target_feature.makeShared(),
					source2target_);
			findCorrespondences(target_feature.makeShared(), source_features_,
					target2source_);
	...
	std::cout << "Max Matching Object for  " << label << " is: "
			<< matching_object << " with: " << max_num_corr << std::endl;
...
template<typename FeatureType>
std::pair<pcl::PointCloud<FeatureType>, pcl::PointCloud<pcl::PointXYZI> > ObjectRecognition<
		FeatureType>::computeAndStoreDescriptor(
		typename pcl::PointCloud<pcl::PointXYZRGBA>::ConstPtr source,
		std::string filename) {
...
template<typename FeatureType>
std::pair<pcl::PointCloud<FeatureType>, pcl::PointCloud<pcl::PointXYZI> > ObjectRecognition<
		FeatureType>::loadDescriptor(const std::string filename)

{

	pcl::PointCloud<FeatureType> feature_desc;
	pcl::PointCloud<pcl::PointXYZI> keypoints;
	if (pcl::io::loadPCDFile<FeatureType>(filename, feature_desc) == -1) //* load the file
			{
...
void ObjectRecognition<FeatureType>::extractDescriptors(
	...
	typename pcl::FeatureFromNormals<pcl::PointXYZRGBA, pcl::Normal, FeatureType>::Ptr feature_from_normals =
			boost::dynamic_pointer_cast<
					pcl::FeatureFromNormals<pcl::PointXYZRGBA, pcl::Normal,
							FeatureType> >(feature_extractor_);
	...
	{
		if (debug_)
			std::cout << "normal estimation..." << input->height << " " << input->width << " " << input->size() << std::flush;
...
void ObjectRecognition<FeatureType>::filterCorrespondences() {
	...
	if (debug_) {
		std::cout << "Size of filtered correspondences " << correspondences_->size();
		std::cout		<< std::endl;
		std::cout << "OK" << std::endl;
	}
}
...
void ObjectRecognition<FeatureType>::determineInitialTransformation() {
	...
	transformation_estimation->estimateRigidTransformation(*source_keypoints_,
			*target_keypoints_, *correspondences_,
...
std::pair<int, int> ObjectRecognition<FeatureType>::matchToDatabase(
	...
	typename std::pair<pcl::PointCloud<FeatureType>,
			pcl::PointCloud<pcl::PointXYZI> > feature_keypoint;
	int max_num_corr = 0, matching_object = -1;
	...
	std::pair<int, int> corr_object = matchToFeatureFile(cluster, label, covariance_mat);
	...
	std::string pose_filename = pose_directory_ + "/object_" +  boost::lexical_cast<std::string>(label);
	...
	if (max_num_corr < 12) /* store in the file */
	{
		...
		std::string output_filename = feature_directory_ + "/object_"
...
template<typename FeatureType>
PoseVector ObjectRecognition<FeatureType>::loadPoseArray(
		std::string filename) {
...
int ObjectRecognition<FeatureType>::loadDatabase(std::string dir) {
	...
	for (boost::filesystem::directory_iterator itr(feature_directory_); itr != end_itr;
			++itr) {
		...
		if (string_find != std::string::npos)
			continue;
	...
	//			<< feature_centroid[2] << std::endl;
		...
		//std::cout << "Feature Centroid of " << itr->path().string() << "  is: " << feature_centroid << std::endl;
	...
	for (boost::filesystem::directory_iterator itr(pose_directory_); itr != end_itr;
			++itr) {
		/* load object templates and save it in feature_desc vector */
...
/*	std::cout << "Loading models" << std::endl;
	for (boost::filesystem::directory_iterator itr(model_directory_); itr != end_itr;
			++itr) {
		 load object templates and save it in feature_desc vector
...
int ObjectRecognition<FeatureType>::loadMapping(std::string filename){
	...
	int segment_id, object_id, object_count;
	  if (file.is_open())
	  {
	   ...
	   //   for(std::map<int,int>::iterator it = segment_object[segment_id].begin(); it != segment_object[segment_id].end(); it++){
	  //  	  std::cout << it->first << " " << it->second << std::endl;
	  //    }

	      if(segment_id>max_segment)
	    	  max_segment=segment_id;
