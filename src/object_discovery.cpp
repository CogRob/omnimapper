...
void ObjectDiscovery<PointT>::mergeClouds() {
	...
	boost::filesystem::path path_to_remove(
			"/home/siddharth/kinect/merged_object_models/");
	for (boost::filesystem::directory_iterator end_dir_it, it(path_to_remove);
			it != end_dir_it; ++it) {
		remove_all(it->path());
	}
	boost::filesystem::path path_to_remove_1(
			"/home/siddharth/kinect/merged_object_templates/");
	for (boost::filesystem::directory_iterator end_dir_it, it(path_to_remove_1);
			it != end_dir_it; ++it) {
		remove_all(it->path());
	}
	boost::filesystem::path path_to_remove_2(
			"/home/siddharth/kinect/merged_pose_templates/");
	...
	for (it = object_map.begin(); it != object_map.end(); it++) {
		...
		// save merged clouds
		std::string output_file = "/home/siddharth/kinect/merged_object_models/"
		...
		std::string pose_filename = "/home/siddharth/kinect/merged_pose_templates/object_"
		...
		std::string output_filename = "/home/siddharth/kinect/merged_object_templates/object_"
	...
	correspondence_estimator->saveMapping(
			"/home/siddharth/kinect/merged_mapping.txt");
	correspondence_estimator->saveObjectStats(
			"/home/siddharth/kinect/merged_stats.txt");
