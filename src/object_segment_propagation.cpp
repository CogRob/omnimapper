...
SegmentPropagation<PointT>::SegmentPropagation () :
    verbose_ (true), debug_ (true), max_size (0), prev_cloud_vec_ (), final_cloud_vec_ (), final_count (), active_label_indices (), prev_labels_ (), restart_flag (
        false), final_map_cloud ()
{
  ...
  if (debug_)
  {
    ...
    std::cout << "Size of final_cloud_vec " << final_cloud_vec_.size ()
        << std::endl;
    std::cout << "Size of active label indices " << active_label_indices.size ()
        << std::endl;
...
template<typename PointT>
typename SegmentPropagation<PointT>::CloudPtrVector SegmentPropagation<
    PointT>::copyLabel (CloudPtrVector label, CloudPtrVector final_label)
{
  ...
  for (int i = 0; i < final_label.size (); i++)
  {
    ...
    {
      std::cout << "Final label " << final_label[i]->points.size ()
          << std::endl;
...
template<typename PointT>
typename SegmentPropagation<PointT>::CloudPtrVector SegmentPropagation<
    PointT>::initializeLabel (CloudPtrVector label)
{
  ...
  if (active_label_indices.size () == 0)
  {
    ...
    {
      active_label_indices[i] = 1;
    }
    return label;
  }
  std::map<int, int>::reverse_iterator it;
  it = active_label_indices.rbegin ();
  ...
  for (int i = 0; i < label.size (); i++)
  {
    ...
    active_label_indices[i + active_size + 1] = 1;
  }
    ...
    std::vector<pcl::PointIndices> label_indices, CloudConstPtr cloud_ptr)
{
  ...
  for (int idx = 0; idx < label_indices.size (); idx++)
  {
            ...
            < cloud_ptr->height * cloud_ptr->width)
    {
      ...
      for (int i = 0; i < label_indices[idx].indices.size (); i++)
      {
    ...
    CloudPtrVector label)
{
  ...
  for (int idx = 0; idx < label.size (); idx++)
  {
    ...
    if (label[idx]->points.size () > SEGMENT_SIZE)
    {
      ...
      // compute euclidean distance centroid and color centroid
      for (int i = 0; i < label[idx]->points.size (); i++)
      {
        int coord = i;
    ...
    std::vector<float> curr_centroids, std::vector<float> prev_centroids)
{
  ...
  int num_edges = curr_centroids.size () / NUM_PARAMS;  // number of edges in bipartite matching graph
  ...
  {
    if (curr_centroids[idx] == -1)
      continue;
    float min = 1000000000, min_diff_num = 0;
    int nn_index = -1;
    for (int i = 0; i < prev_centroids.size (); i += NUM_PARAMS)
    {
      if (prev_centroids[i] == -1)
        continue;
    ...
    std::map<int, std::vector<int> > rev_neighbor_vec, int label_size)
{
  ...
  for (size_t idx = 0; idx < neighbor_vec.size (); idx++)
  {
    ...
    if (neighbor_vec[idx] != -1 && neighbor_vec[idx] != -2)
    {  // size of the segment is less than SEGMENT_SIZE
       // std::cout << "Index: " << i << " Neighbor: " << neighbor_vec[i] << " Size of rev NN: " << rev_neighbor_vec[neighbor_vec[i]].size() << std::endl;
      int nn_index = neighbor_vec[idx];

      if (rev_neighbor_vec[nn_index].size () == 1)
      {
        final_neighbor_vec[idx] = nn_index;
        active_label_indices[nn_index] = 1;
        if (debug_)
      ...
      for (int j = 0; j < rev_neighbor_vec[nn_index].size (); j++)
      {
        ...
        float jaccard_index = computeJaccardIndex (curr_centroids,
            prev_centroids, idy * NUM_PARAMS, nn_index * NUM_PARAMS);

        float diff_eq = sqrtf (diffX * diffX + diffY * diffY + diffZ * diffZ);

        //float diff_eq = -jaccard_index;
        float diff_color = 0;  //sqrtf(diffR*diffR + diffG*diffG + diffB*diffB);
        // std::cout << i << " " << prev_centroids[i] << " " << diff_eq << " " << min << " " << nn_index << std::endl;
        if (debug_)
        ...
        if ( ( (diff_eq + diff_color) / 2) < min)
        {
          min = ( (diff_eq + diff_color) / 2);
          nn_nn_index = idy;
          //       std::cout << "Index: " << idx << " " << nn_index << " " << diff_eq << "Prev Centroids " << prev_centroids[i] << " " << prev_centroids[i+1] << " " << prev_centroids[i+2] << std::endl;
        }
      ...
      for (int j = 0; j < rev_neighbor_vec[nn_index].size (); j++)
      {
        ...
        if (idy != nn_nn_index)
        {
          ...
          for (int k = 0; k < label_size, flag_check == 0; k++)
          {
            ...
            // if label K in the previous frame didn't match with any of the label in the current frame

            if (rev_neighbor_vec.find (k) == rev_neighbor_vec.end ()
                && active_label_indices.find (k) == active_label_indices.end ())
            {
              // found a label
            ...
            else
            {
              if (debug_)
              {
                ...
                if (rev_neighbor_vec.find (k) == rev_neighbor_vec.end ())
                  ...
                  std::cout << "1" << std::endl;
                if (active_label_indices.find (k)
                    == active_label_indices.end ())
                  std::cout << "0" << std::endl;
                else
                  std::cout << "1" << std::endl;
              }
            ...
            && active_label_indices.find (k) == active_label_indices.end ())
        {
          ...
          if (debug_)
          {
            ...
            if (rev_neighbor_vec.find (k) == rev_neighbor_vec.end ())
              ...
              std::cout << "1" << std::endl;
            if (active_label_indices.find (k) == active_label_indices.end ())
              std::cout << "0" << std::endl;
            else
              std::cout << "1" << std::endl;
          }
    ...
    std::vector<pcl::PointIndices> euclidean_label_indices)
{
  ...
  final_label_indices.resize (max_size);
  back_label_.clear ();
  for (size_t idx = 0; idx < neighbor_vec.size (); idx++)
  {
    ...
    for (int j = 0; j < euclidean_label_indices[idx].indices.size (); j++)
    {
...
template<typename PointT>
typename SegmentPropagation<PointT>::CloudPtrVector SegmentPropagation<
    PointT>::findFinalLabels (std::vector<int> neighbor_vec,
    ...
    CloudConstPtr prev_cloud_ptr_, CloudConstPtr curr_cloud_ptr_)
{
  ...
  max_size =
      (prev_label_indices_.size () > curr_label_indices_.size ()) ?
          prev_label_indices_.size () : curr_label_indices_.size ();

  //std::cout << "Size of Prev label indices " << prev_label_indices_.size() << " Size of Current Label Indices: " << euclidean_label_indices.size() << std::endl;
  std::vector<float> prev_centroids;
  prev_centroids = computeCentroids (prev_label_indices_, prev_cloud_ptr_);

  if (verbose_)
    std::cout << "Prev Centroids Computed..." << std::endl;

  std::vector<float> curr_centroids;
  curr_centroids = computeCentroids (curr_label_indices_, curr_cloud_ptr_);

  if (verbose_)
    std::cout << "Current Centroids Computed..." << std::endl;
  ...
  //std::vector<int> neighbor_vec_forward = linearMatch(prev_centroids, curr_centroids);
...
//	std::cout << "Linear Matching ... " << std::endl;
  ...
  if (verbose_)
    std::cout << "Created Reverse Neighbor vec..." << std::endl;

  //two way match

  final_neighbors = twoWayMatch (prev_centroids, curr_centroids, neighbor_vec,
      rev_neighbor_vec, curr_label_indices_.size ());

  if (verbose_)
    std::cout << "Two way match completed..." << std::endl;

  // find final labels
  std::vector<pcl::PointIndices> final_label_indices;
  final_label_indices = findFinalLabels (final_neighbors, curr_label_indices_);
  ...
  if (debug_)
    for (size_t idx = 0; idx < neighbor_vec.size (); idx++)
    {
      ...
      float diff_num = fabs (
          curr_centroids[idx + 6] - prev_centroids[nn_index + 6]);
...
template<typename PointT>
typename SegmentPropagation<PointT>::CloudPtrVector SegmentPropagation<
    PointT>::propagateLabels (CloudPtrVector prev_label_indices_,
    CloudPtrVector curr_label_indices_)
{
  ...
  if (verbose_)
    std::cout << "Size of Prev label indices " << prev_label_indices_.size ()
        << " Size of Current Label Indices: " << curr_label_indices_.size ()
        << std::endl;
  std::vector<float> prev_centroids;
...
//	std::cout << "Linear Matching ... " << std::endl;
  ...
  //two way match
  std::vector<int> final_neighbor_vec = twoWayMatch (prev_centroids,
      curr_centroids, neighbor_vec, rev_neighbor_vec,
  ...
  // find final labels
  CloudPtrVector final_label_indices;
  final_label_indices = findFinalLabels (final_neighbor_vec,
      curr_label_indices_);
  ...
  if (debug_)
  {
    ...
    std::cout << "Size of final label indices: " << final_label_indices.size ()
        << std::endl;
    for (int i = 0; i < final_label_indices.size (); i++)
    {
  ...
  final_neighbors = final_neighbor_vec;
...
template<typename PointT>
typename SegmentPropagation<PointT>::CloudPtrVector SegmentPropagation<
    PointT>::propagateLabels (CloudPtrVector label, gtsam::Pose3 pose_,
    gtsam::Symbol sym)
{
  ...
  if (prev_cloud_vec_.size () == 0 || restart_flag)
  {
    ...
    std::cout << "[initializing] PrevCloudPtrVector is empty:  "
        << prev_cloud_vec_.size () << " CurrentCloudPtrVector size: "
        << label.size () << std::endl;
  ...
  observations_.insert (
      std::pair<gtsam::Symbol, CloudPtrVector> (sym,
          final_untransformed_label));
...
template<typename PointT>
typename SegmentPropagation<PointT>::CloudPtrVector SegmentPropagation<
    PointT>::predictLabels (CloudPtrVector label, gtsam::Pose3 pose_,
    gtsam::Symbol sym)
{
  ...
  if (prev_cloud_vec_.size () == 0 || restart_flag)
  {
    ...
    std::cout << "Size of final_label = " << final_label.size ()
        << " Size of untransformed label " << final_untransformed_label.size ()
        << std::endl;
    prev_cloud_vec_ = final_label;
  ...
  std::cout << "Adding symbol " << " and cloud of size "
      << final_untransformed_label.size () << std::endl;
  observations_.insert (
      std::pair<gtsam::Symbol, CloudPtrVector> (sym,
          final_untransformed_label));
  final_map_cloud = createFinalCloud (sym, pose_);
  end = pcl::getTime ();
  std::cout << "Final cloud creation took: " << double (end - start)
      << std::endl;

  std::cout << "Size of final map cloud " << final_map_cloud.size ()
      << std::endl;
...
template<typename PointT> typename SegmentPropagation<PointT>::CloudPtrVector SegmentPropagation<
    PointT>::createFinalCloud (gtsam::Symbol sym_latest, gtsam::Pose3 cloud_pose)
{
  ...
  for (it = observations_.begin(); it != observations_.end(); it++)
  {
    ...
    if (cloud_pose)
    {
      ...
      for (int j = 0; j < cloud.size(); j++)
      {
        ...
        if (final_cloud_opt_.size() <= j)
        final_cloud_opt_.push_back(map_cloud);
        if (cloud[j]->points.size() == 0)
        continue;
...
#else
  typename std::vector<CloudPtr> final_cloud_opt_;
  final_cloud_opt_ = final_map_cloud;
