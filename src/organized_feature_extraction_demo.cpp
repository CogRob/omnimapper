...
class OrganizedFeatureExtractionDemo
{
  public:
    typedef pcl::PointCloud<PointT> Cloud;
    typedef typename Cloud::Ptr CloudPtr;
  ...
  public:
    OrganizedFeatureExtractionDemo ()
...
main (int argc, char** argv)
{
  ...
  omnimapper::OrganizedFeatureExtraction<PointT> ofe (ni_grabber);

  OrganizedFeatureExtractionDemo<PointT> demo;
  boost::function<void(const CloudConstPtr&, const LabelCloudConstPtr&)> label_callback = boost::bind (&OrganizedFeatureExtractionDemo<PointT>::clusterLabelsCallback, &demo, _1, _2);
