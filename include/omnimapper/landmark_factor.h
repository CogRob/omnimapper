...
class LandmarkTransformFactor : public NoiseModelFactor2<POSE, LANDMARK> {
 ...
 public:
  ...
  LandmarkTransformFactor(Key poseKey, Key pointKey, const LANDMARK& measured,
  ...
  virtual Vector evaluateError(
      const POSE& pose, const LANDMARK& point,
      boost::optional<Matrix&> H1 = boost::none,
      boost::optional<Matrix&> H2 = boost::none) const {
    // Transform the point and calculate the error. Jacobians are computed
    // internally.
    return (measured_.localCoordinates(pose.transform_to(point, H1, H2)));
  }

  const LANDMARK& measured() const { return (measured_); }
