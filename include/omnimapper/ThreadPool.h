...
// Downloaded from
// https://raw.githubusercontent.com/log4cplus/ThreadPool/master/ThreadPool.h
...
inline ThreadPool::ThreadPool(std::size_t threads)
  ...
  for (std::size_t i = 0; i != threads; ++i) emplace_back_worker(i);
}
...
inline void ThreadPool::set_pool_size(std::size_t limit) {
  ...
  if (pool_size > old_size) {
    ...
    for (std::size_t i = old_size; i != pool_size; ++i) emplace_back_worker(i);
