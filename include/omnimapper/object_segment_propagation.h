...
class SegmentPropagation
{
  public:
        ...
        CloudConstPtr curr_cloud_ptr_)
    {
      ...
      {
        PCL_INFO(
            "Previous Cloud is Empty!! Saving the current cloud for future propagation\n");
        ...
        pcl::PointCloud<pcl::Label> labels, CloudConstPtr curr_cloud_ptr_)
    {
      ...
      {
        if (labels.points[i].label > labels.points.size ())
          continue;
      ...
      if (final_label_indices_.size () == 0)
        return final_labels;
      ...
      for (int i = 0; i < final_label_indices_.size (); i++)
      {
        ...
        for (int j = 0; j < final_label_indices_[i].indices.size (); j++)
        {
          ...
          if (labels.points[pt].label > labels.points.size ())
            continue;
        ...
        CloudPtrVector label)
    {
      ...
      final_label_indices = findFinalLabels (final_neighbors,
    ...
    std::vector<int> twoWayMatch (std::vector<float> prev_centroids,
        std::vector<float> curr_centroids, std::vector<int> neigbhor_vec,
    ...
    float computeJaccardIndex (std::vector<float> curr_centroids,
        std::vector<float> prev_centroids, float idx, float nn_idx);

    CloudPtrVector createFinalCloud (gtsam::Symbol sym, gtsam::Pose3 pose);

    bool restart_flag;  // initialize new labels -- time gap between previous and current frame is large
