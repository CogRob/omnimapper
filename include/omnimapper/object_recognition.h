...
private:
	...
	//ar &  boost::serialization::base_object<std::vector<gtsam::Pose3> >(*this);
		ar & pose_vector;
}
...
public:
	...
	void storeDescriptor(typename pcl::PointCloud<FeatureType>::Ptr features,
			std::string filename);
	std::pair<pcl::PointCloud<FeatureType>, pcl::PointCloud<pcl::PointXYZI> > loadDescriptor(
	...
	int matchToFile(
			typename pcl::PointCloud<pcl::PointXYZRGBA>::ConstPtr source, typename pcl::PointCloud<pcl::PointXYZRGBA>::ConstPtr target);

	std::pair<int, int> matchToFeatureFile(
			typename pcl::PointCloud<pcl::PointXYZRGBA>::ConstPtr source, int label, gtsam::Matrix covariance);

	std::pair<pcl::PointCloud<FeatureType>, pcl::PointCloud<pcl::PointXYZI> > computeAndStoreDescriptor(
			typename pcl::PointCloud<pcl::PointXYZRGBA>::ConstPtr source,
			std::string filename);

	std::pair<int, int> matchToDatabase(
			typename pcl::PointCloud<pcl::PointXYZRGBA>::ConstPtr cluster, std::vector<gtsam::Pose3> pose_array,
			int label, gtsam::Matrix covariance);
...
//                    pcl::visualization::PCLVisualizer visualizer_;
	...
	//boost::shared_ptr<pcl::PCLSurfaceBase<pcl::PointXYZRGBANormal> > surface_reconstructor_;
	...
	std::string feature_directory_, base_directory_, pose_directory_, model_directory_;

	typename std::map<int, std::vector<pcl::PointCloud<FeatureType> > > feature_map;
