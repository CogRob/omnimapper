 ...
 * @author Zhaoyang Lv
 * @brief An infinite plane, represented by a normal direction and perpendicular
 * distance
 */
 ...
 * normal and its perpendicular distance to the origin. Currently it provides a
 * transform of the plane, and a norm 1 differencing of two planes. Refer to
 * Trevor12iros for more math details.
 */
class OrientedPlane3 : public gtsam::DerivedValue<OrientedPlane3> {
 ...
 public:
  ...
  /// Default constructor
  OrientedPlane3() : n_(), d_(0.0) {}

  /// Construct from a Unit3 and a distance
  OrientedPlane3(const gtsam::Unit3& s, double d) : n_(s), d_(d) {}
   ...
   */
  OrientedPlane3 transform(
      const gtsam::Pose3& xr, boost::optional<gtsam::Matrix&> Hp = boost::none,
   ...
   * @return the transformed plane
   */
  static OrientedPlane3 Transform(
      const OrientedPlane3& plane, const gtsam::Pose3& xr,
   ...
   * derivatives. NOTE(hayk): The derivatives are zero when normals are exactly
   * orthogonal.
   * @param the other plane
   */
  // gtsam::Vector3 errorVector(const OrientedPlane3& other,
