...
namespace omnimapper
{
  ...
  class PoseChainNode
  {
    public:
      ...
      // void
      // addFactor (gtsam::NonlinearFactor::shared_ptr& factor)
      // {
    ...
    public:
      ...
      Status status;
      // The list of factors to commit for this entry (only used for created and staged status)
      std::vector<gtsam::NonlinearFactor::shared_ptr> factors;
  };

  // /* \brief PoseChain manages the creation of poses for the mapper and the plugins, as well as the addition of poses and attached factors to the mapper. */
  // class PoseChain
  // {
  //   public:
  //     /* Empty Constructor */
  ...
  //             symbol_times.insert (std::pair<Time, std::list<omnimapper::PoseChainNode>::iterator>(t, new_itr));
  //             break;
  //           }
  //         }
  //         return;
  ...
  //       {
  //         printf ("PoseChain: Attempting to add factor at invalid pose key!\n");
  //         return;
  //       }
  //       printf ("NYI\n");
  //     }
  ...
  //     void
  //     addRelativePoseMeasurement (gtsam::BetweenFactor<gtsam::Pose3>::shared_ptr& pose_factor)
  //     {
  //       // Get start and end times
  //       gtsam::Symbol start_sym = pose_factor->front ();
  //       gtsam::Symbol end_sym = pose_factor->end ();

  //       // We require that the start time has been already added to the pose chain
