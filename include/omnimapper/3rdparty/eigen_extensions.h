...
inline void stdToEig(const std::vector<double>& std, Eigen::VectorXd* eig) {
  eig->resize(std.size());
  for (size_t i = 0; i < std.size(); ++i) eig->coeffRef(i) = std[i];
}
...
void save(const Eigen::SparseMatrix<ScalarType, Options, IndexType>& mat,
          const std::string& filename);

template <class ScalarType, int Options, class IndexType>
void load(const std::string& filename,
          Eigen::SparseMatrix<ScalarType, Options, IndexType>* mat);
...
template <class ScalarType, int Options, class IndexType>
void serialize(const Eigen::SparseMatrix<ScalarType, Options, IndexType>& mat,
               std::ostream& strm);
...
void serialize(const Eigen::SparseMatrix<ScalarType, Options, IndexType>& mat,
  ...
  typedef typename Eigen::SparseMatrix<ScalarType, Options,
                                       IndexType>::InnerIterator InnerIterator;
  for (IndexType i = 0; i < mat.outerSize(); ++i) {
    int num = 0;
    for (InnerIterator it(mat, i); it; ++it) ++num;
...
void loadASCII(const std::string& filename, Eigen::Matrix<S, T, U>* mat) {
  ...
  file.open(filename.c_str());
  if (!file)
    std::cerr << "File " << filename << " could not be opened.  Dying badly."
              << std::endl;
  assert(file);
