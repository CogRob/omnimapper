...
CloudPtr createXYZRGBPointCloud(std::string& rgb_filename,
  ...
  // boost::posix_time::ptime cloud_time = time_t_epoch +
  // boost::posix_time::seconds (seconds) + boost::posix_time::microseconds
  // (microseconds);
  ...
  std::cout << "Processing cloud from: " << stamp
            << " with stamp: " << cloud->header.stamp << std::endl;
  ...
  for (int r = 0; r < cloud->height; r++) {
    for (int c = 0; c < cloud->width; c++) {
      ...
      float Z =
          static_cast<float>(depth.at<unsigned short>(r, c)) / scalingFactor;
      cloud->points[idx].x =
          (static_cast<float>(c) - centerX) * Z / focalLength;
      cloud->points[idx].y =
          (static_cast<float>(r) - centerY) * Z / focalLength;
...
int main(int argc, char** argv) {
  ...
  if (argc < 3 || help) {
    ...
    std::cout << "(e.g.) " << argv[0] << " associate.txt pose.txt time.txt"
              << std::endl;
    return (-1);
  }
  ...
  for (int i = 0; i < numof_total_frames; i++) {
    CloudPtr cloud_ptr =
        createXYZRGBPointCloud(rgb_files[i], depth_files[i], stamps[i]);
    ...
    // boost::posix_time::microsec_clock::local_time (); eval->process
    // (stamps[i], cloud_ptr); boost::posix_time::ptime current_time =
    // boost::posix_time::microsec_clock::local_time (); double time =
    // static_cast<double>((current_time - start_time).total_nanoseconds ()) *
    // 1e-6; std::cout << "[" << i+1 << "/" << numof_total_frames << "] in
    // processing... takes " << time << " ms..." << std::endl;
