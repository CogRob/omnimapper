...
int main(int argc, char** argv) {
  ...
  // ICP Score Threshold
  double score_thresh_min = 0.1;
  double score_thresh_max = 1.0;
  double score_thresh_step = 0.2;
  ...
  for (std::size_t i = 0; i < datasets.size(); i++) {
    ...
    std::string associated_filename =
        base_path + std::string("/associated.txt");
    std::string ground_truth_filename =
        base_path + std::string("/groundtruth.txt");
    ...
    for (double score_thresh = score_thresh_min;
         score_thresh <= score_thresh_max; score_thresh += score_thresh_step) {
      ros::param::set("/benchmark_node/icp_score_thresh", score_thresh);

      // for (double correspondence_dist = correspondence_dist_min;
      // correspondence_dist <= correspondence_dist_max; correspondence_dist +=
      // correspondence_dist_step)
      //{
      double correspondence_dist = 0.1;
      ros::param::set("/benchmark_node/icp_correspondence_distance",
                      correspondence_dist);

      // Create an output filename
      char output_name[4096];
      sprintf(output_name, "trajectory_%s_scorethresh_%lf_corrdist_%lf.txt",
              datasets[i].c_str(), score_thresh, correspondence_dist);
      std::string output_trajectory_filename(
          "/home/atrevor/benchmark/icp_lc/trajectories/");
      output_trajectory_filename.append((std::string(output_name)));
      std::string output_timing_filename(
          "/home/atrevor/benchmark/icp_lc/timing/");
      ...
      std::cout << "Output Trajectory filename: " << output_trajectory_filename
                << std::endl;
  ...
  // std::string associated_filename
  // ("/home/atrevor/data/tum_rgbd/rgbd_dataset_freiburg1_desk/associated.txt");
  // std::string ground_truth_filename
  // ("/home/atrevor/data/tum_rgbd/rgbd_dataset_freiburg1_desk/groundtruth.txt");
  // std::string pcd_path
  // ("/home/atrevor/data/tum_rgbd/rgbd_dataset_freiburg1_desk/pcd/rgb");
  // std::string output_filename
  // ("/home/atrevor/data/tum_rgbd/rgbd_dataset_freiburg1_desk/output.txt");
  // std::string output2_filename
  // ("/home/atrevor/data/tum_rgbd/rgbd_dataset_freiburg1_desk/output2.txt");

  // omnimapper.runEvaluation (associated_filename, ground_truth_filename,
  // pcd_path, output_filename); omnimapper.runEvaluation (associated_filename,
  // ground_truth_filename, pcd_path, output2_filename);
