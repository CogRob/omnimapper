...
int main (int argc, char** argv)
{
  ...
  for (int i = 0; i < datasets.size (); i++)
  {
    ...
    for (double score_thresh = score_thresh_min; score_thresh <= score_thresh_max; score_thresh += score_thresh_step)
    {
      ...
      sprintf (output_name, "trajectory_%s_scorethresh_%lf_corrdist_%lf.txt", datasets[i].c_str (), score_thresh, correspondence_dist);
      std::string output_trajectory_filename ("/home/atrevor/benchmark/icp_lc/trajectories/");
      output_trajectory_filename.append ((std::string (output_name)));
      std::string output_timing_filename ("/home/atrevor/benchmark/icp_lc/timing/");
