...
int main(int argc, char** argv) {
  ...
  for (std::size_t i = 0; i < datasets.size(); i++) {
    ...
    // For each leaf size
    for (double leaf_size = leaf_size_min; leaf_size <= leaf_size_max;
         leaf_size += leaf_size_step) {
      ...
      sprintf(output_name, "trajectory_%s_leafsize_%lf_corrdist_%lf.txt",
              datasets[i].c_str(), leaf_size, correspondence_dist);
      std::string output_trajectory_filename(
          "/home/atrevor/benchmark/icp_seq/trajectories/");
      output_trajectory_filename.append((std::string(output_name)));
      std::string output_timing_filename(
          "/home/atrevor/benchmark/icp_seq/timing/");
