...
void omnimapper::OmniMapperVisualizerRViz<PointT>::spinOnce() {
  ...
  if (draw_pose_graph_) {
    ...
    BOOST_FOREACH (const gtsam::NonlinearFactorGraph::sharedFactor& factor,
                   (*current_graph)) {
      // check for poses
      const std::vector<gtsam::Key> keys(factor->keys().begin(),
                                         factor->keys().end());
  ...
  if (draw_planar_landmarks_) {
    ...
    int id = 0;
    BOOST_FOREACH (
        const typename gtsam::Values::ConstFiltered<
...
void omnimapper::OmniMapperVisualizerRViz<PointT>::objectCallback(
  ...
  for (it = object_map.begin(); it != object_map.end(); it++) {
    ...
    top_objects.insert(
        std::pair<int, gtsam::Symbol>(it->second.clusters_.size(), sym));
  }
  ...
  gtsam::Point3 frame_center =
      transformed_view_center + depth_limit * view_direction;
