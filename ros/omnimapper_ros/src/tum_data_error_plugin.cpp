...
omnimapper::TUMDataErrorPlugin::TUMDataErrorPlugin (omnimapper::OmniMapperBase* mapper)
  : nh_ ("~"),
    tf_listener_ (ros::Duration (180.0)),
    mapper_ (mapper)
{
...
void
omnimapper::TUMDataErrorPlugin::update (boost::shared_ptr<gtsam::Values>& vis_values, boost::shared_ptr<gtsam::NonlinearFactorGraph>& vis_graph)
{
  ...
  int count = 0;
  gtsam::Values::ConstFiltered<gtsam::Pose3> pose_filtered = vis_values->filter<gtsam::Pose3>();
  BOOST_FOREACH (const gtsam::Values::ConstFiltered<gtsam::Pose3>::KeyValuePair& key_value, pose_filtered)
  {
    ...
    tf_listener_.waitForTransform ("/world", "/openni_rgb_optical_frame", pose_time, ros::Duration (0.2));
    tf_listener_.lookupTransform ("/world", "/openni_rgb_optical_frame", pose_time, true_pose);
    ...
    printf ("adding point: %lf %lf %lf\n", true_point_msg.x, true_point_msg.y, true_point_msg.z);
