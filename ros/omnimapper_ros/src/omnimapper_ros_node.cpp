...
class OmniMapperROSNode
{
  public:
    ...
    OmniMapperROSNode () :
        n_ ("~"), omb_ (), tf_plugin_ (&omb_), no_motion_plugin_ (&omb_), icp_plugin_ (
            &omb_), edge_icp_plugin_ (&omb_), plane_plugin_ (&omb_), object_plugin_ (
            &omb_), csm_plugin_ (&omb_), vis_plugin_ (&omb_), csm_vis_plugin_ (
            &omb_), tsdf_plugin_ (&omb_), bag_error_plugin_ (&omb_), eval_plugin_ (
            &omb_), fake_grabber_ (empty_files_, 1.0, false), organized_feature_extraction_ (
            fake_grabber_), tf_listener_ (ros::Duration (500.0))
    {
      // Load some params
