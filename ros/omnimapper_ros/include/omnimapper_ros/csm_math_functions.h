 ...
 *
 * \param last_pose   The previous pose where a message from this sensor message was used to augment the map
 * \param curr_pose   The current pose of the robot
 * \param distance    The required displacement for the new message to be considered independent enough to incorporate into the map
 * \return true if this displacement is sufficient, meaning that this message should be used.  If this is false, then it shouldnt be used.  This method ignores rotation
 *
 */
 ...
 *
 *\param transform   The TF::StampedTransform where the robot is to get the pose from
 *
...
/*!
 * \brief Gets the relative pose between two transformations, used to compute the odometric pose between two map robot poses
 *
 *\param last_pose   The last (odometric)pose of the robot, where the prior message was used
 *\param curr_pose   The current (odometric)pose of the robot
 * \return the pose transforming last_pose into curr_pose, which is the measurement for the Pose2Factor
 */
