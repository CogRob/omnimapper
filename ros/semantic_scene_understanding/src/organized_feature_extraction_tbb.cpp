...
namespace omnimapper
{
  ...
  min_cluster_inliers_ (1000),
    ...
    OrganizedFeatureExtractionTBB<PointT>::cloudCallback (const CloudConstPtr& cloud)
    {
      // Store cloud
      boost::mutex::scoped_lock (cloud_mutex);
      {
        //boost::lock_guard<boost::mutex> lock (cloud_mutex);
        //std::cout << "OrganizedFeatureExtractionTBB: Cloud stamp: " << cloud->header.stamp << std::endl;
  ...
  template <typename PointT> void
  OrganizedFeatureExtractionTBB<PointT>::publish ()
  {
    ...
    if (cluster_cloud_callbacks_.size () > 0)
    {
      ...
      {
        Time timestamp = boost::posix_time::microsec_clock::local_time();//stamp2ptime ((*pub_cluster_cloud_)->header.stamp);
    ...
    OrganizedFeatureExtractionTBB<PointT>::processFrame ()
    {
      while (true)
      {
        ...
        else
        {
          ...
          boost::thread ne_thread (&OrganizedFeatureExtractionTBB::computeNormals, boost::ref (this));
          //ne_thread.join ();
          ...
          boost::thread mps_thread (&OrganizedFeatureExtractionTBB::computePlanes, boost::ref (this));
          
          // Extract edges
          oed.setInputCloud (stage2_cloud_);
          double oed_thread_spawn = pcl::getTime ();
          boost::thread oed_thread (&OrganizedFeatureExtractionTBB::computeEdges, boost::ref (this));
          
          // Compute Euclidean Clusters
          double clust_thread_spawn = pcl::getTime ();
          boost::thread clust_thread (&OrganizedFeatureExtractionTBB::computeClusters, boost::ref (this));
          ...
          //   std::cout << "Stage3 labels: " << mps_output_labels_->points.size () << std::endl;
          ...
          if (planar_region_stamped_callbacks_.size () > 0)
          {
            ...
            std::cout << "stage3 regions has: " << mps_output_regions_.size () << std::endl;
            ...
            if (stage2_cloud_->points.size () > 200)
            {
              for (int i = 0; i < planar_region_stamped_callbacks_.size (); i++)
                planar_region_stamped_callbacks_[i] (mps_output_regions_, timestamp);
            }
          }
          ...
          // alt
          {
            ...
            std::cout << "stage3 stamp: " << mps_output_cloud_->header.stamp << std::endl;
            stage4_cloud_.swap (mps_output_cloud_);
            mps_output_cloud_.swap (stage2_cloud_);
            ...
            std::cout << "stage3 stamp after: " << mps_output_cloud_->header.stamp << std::endl;
...
*/
    ...
    // Compute the normals
  template <typename PointT> void
  OrganizedFeatureExtractionTBB<PointT>::computeNormals ()
  {
    if (!input_cloud_)
    {
      ne_output_cloud_ = boost::none;
  ...
  // Compute clusters
  template <typename PointT> void
  OrganizedFeatureExtractionTBB<PointT>::computeClusters ()
  {
  ...
  // Compute planes
  template <typename PointT> void
  OrganizedFeatureExtractionTBB<PointT>::computePlanes ()
  {
    ...
    // std::cout << "mps_output_labels_ size: " << (*mps_output_labels_)->points.size () << std::endl;
...
*/
    ...
    template <typename PointT> void
    OrganizedFeatureExtractionTBB<PointT>::setPlanarRegionStampedCallback (boost::function<void (std::vector<pcl::PlanarRegion<PointT>, Eigen::aligned_allocator<pcl::PlanarRegion<PointT> > >, Time)>& fn)
    {
    ...
    template <typename PointT> void
    OrganizedFeatureExtractionTBB<PointT>::setOccludingEdgeCallback (boost::function<void (const CloudConstPtr&)>& fn)
    {
      occluding_edge_callback_ = fn;
    }

  template <typename PointT> void
  OrganizedFeatureExtractionTBB<PointT>::setPlaneLabelsCallback (boost::function<void (const CloudConstPtr&, const LabelCloudConstPtr&)>& fn)
  {
    plane_label_cloud_callback_ = fn;
  }

  template <typename PointT> void
  OrganizedFeatureExtractionTBB<PointT>::setClusterLabelsCallback (boost::function<void (const CloudConstPtr&, const LabelCloudConstPtr&)>& fn)
  {
    cluster_label_cloud_callbacks_.push_back(fn);
  }

  template <typename PointT> void
  OrganizedFeatureExtractionTBB<PointT>::setRegionCloudCallback (boost::function<void(const CloudConstPtr&, std::vector<pcl::PlanarRegion<PointT>, Eigen::aligned_allocator<pcl::PlanarRegion<PointT> > >&)>& fn)
  {
    region_cloud_callback_ = fn;
  }

  template <typename PointT> void
  OrganizedFeatureExtractionTBB<PointT>::setClusterCloudCallback (boost::function<void(std::vector<CloudPtr>, Time,  boost::optional<std::vector<pcl::PointIndices> > )> fn)
  {
